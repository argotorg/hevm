...
rootDirectory = do
  ...
  let testsDir = "BlockchainTests/GeneralStateTests/VMTests/vmIOandFlowOperations/"
  -- let testsDir = "BlockchainTests/GeneralStateTests/VMTests/"
  pure $ repo </> testsDir

collectJsonFiles :: FilePath -> IO [FilePath]
collectJsonFiles rootDir = Find.find Find.always (Find.extension Find.==? ".json") rootDir
-- collectJsonFiles rootDir = Find.find Find.always (Find.fileName Find.==? "codecopy.json") rootDir
  ...
  where
    executionContextFromCase = (executionContext, accounts)
      where
        tx = test.tx
        block = test.block
        code = lookupCode tx.toAddr test.checkContracts
        transaction = CE.Transaction (test.origin) (tx.toAddr) (CE.CallValue tx.value) (CE.CallData tx.txdata) (CE.RuntimeCode <$> code) (CE.Gas tx.gasLimit) (fromJust tx.gasPrice) (priorityFee tx block.baseFee)
        ...
        lookupCode :: Maybe Addr -> BlockchainContracts -> Maybe BS.ByteString
        lookupCode maybeAddr contracts = do
          addr <- maybeAddr
          contract <- Map.lookup addr contracts
          let (BlockchainContract (ByteStringS bs) _ _ _) = contract
          pure bs
