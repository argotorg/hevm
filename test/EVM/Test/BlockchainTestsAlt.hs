...
rootDirectory = do
  ...
  -- let testsDir = "BlockchainTests/GeneralStateTests/stSStoreTest/"
  pure $ repo </> testsDir

collectJsonFiles :: FilePath -> IO [FilePath]
collectJsonFiles rootDir = Find.find Find.always (Find.extension Find.==? ".json") rootDir
-- collectJsonFiles rootDir = Find.find Find.always (Find.fileName Find.==? "sstore_0to0.json") rootDir
  ...
  where
    executionContextFromCase = (executionContext, accounts)
      where
        tx = test.tx
        block = test.block
        transaction = CE.Transaction (test.origin) (tx.toAddr) (CE.CallValue tx.value) (CE.CallData tx.txdata) (CE.Gas tx.gasLimit) (fromJust tx.gasPrice) (priorityFee tx block.baseFee)
