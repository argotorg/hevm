...
rootDirectory = do
  ...
  let testsDir = "BlockchainTests/GeneralStateTests/VMTests/vmTests"
  pure $ repo </> testsDir

collectJsonFiles :: FilePath -> IO [FilePath]
-- collectJsonFiles rootDir = Find.find Find.always (Find.extension Find.==? ".json") rootDir
collectJsonFiles rootDir = Find.find Find.always (Find.fileName Find.==? "blockInfo.json") rootDir
  ...
  where
    executionContextFromCase = (executionContext, accounts)
      where
        ...
        transaction = CE.Transaction (test.origin) (tx.toAddr) (CE.CallValue tx.value) (CE.CallData tx.txdata) (CE.RuntimeCode <$> code) (CE.Gas tx.gasLimit) (fromJust tx.gasPrice)
