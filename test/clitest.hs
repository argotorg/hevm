  ...
  hspec $
    describe "CLI" $ do
      ...
      it "empty solver is always unknown" $ do
        ...
        stdout `shouldContain` "SMT solver returned unknown"
      it "crash-of-hevm" $ do
      ...
      it "dump unsolved" $ do
        ...
        removeFile filename
      it "early-abort" $ do
        (_, stdout, _) <- runForge "test/contracts/pass/early-abort.sol" ["--max-iterations", "1000"]
        stdout `shouldContain` "[FAIL]"
        (T.count "Counterexample:" (T.pack stdout)) `shouldBe` 9
      it "rpc-cache" $ do
