-- | Static analysis for detecting copy loops generated by the Solidity compiler.
--
-- Two kinds of loop are detected:
--
--   * Storage-to-memory (SLOAD → MSTORE): generated for public bytes/string
--     getters. In symbolic execution these appear unbounded and cause
--     MaxIterationsReached.
--
--   * Memory-to-memory (MLOAD → MSTORE): generated e.g. when ABI-encoding a
--     bytes/string return value. When the source buffer is abstract the entire
--     copy can be summarised as a single CopySlice (i.e. b = a).
--
-- Both are detected by the same structural analysis and short-circuited by the
-- symbolic executor.
...
--
-- Accepts two loop kinds:
--   * storage-to-memory: body contains SLOAD and MSTORE/MSTORE8
--   * memory-to-memory: body contains MLOAD and MSTORE/MSTORE8 but no SLOAD
...
analyseBody ops p q = do
  ...
  let isStorageCopy = hasSload && hasMstore
      isMemCopy     = hasMload && hasMstore && not hasSload

  if (isStorageCopy || isMemCopy) && not hasSideEffect && jumpsOK && isStackNeutral
    then Just StorageCopyLoop
           { loopHeadPC  = p
           , loopJumpiPC = q
           , loopExitPC  = q + 1  -- JUMP/JUMPI are single-byte opcodes
           -- Only extract stack layout for mem-to-mem; storage-to-memory loops
           -- are short-circuited (exit taken) without any memory summary.
           , stackLayout = if isMemCopy then extractStackLayout ops p q else Nothing
           }
    else Nothing
...
extractStackLayout ops p q =
  ...
  where
        ...
        OpSwap n ->
          ...
          in if idx < length stk
             then let (top:rest') = stk
                  ...
                  in go rest (target : before ++ [top] ++ after) mloadSrc mstoreA cmpOps
             else Nothing

        OpPop -> go rest (tail stk) mloadSrc mstoreA cmpOps

        -- SLOAD is not tracked here; storage-to-memory loops get stackLayout=Nothing.
        OpSload ->
          go rest (Computed : tail stk) mloadSrc mstoreA cmpOps

        -- Record the source offset consumed by MLOAD.
        OpMload ->
          let addr = head stk
          in go rest (Computed : tail stk) (Just addr) mstoreA cmpOps

        OpMstore ->
          let addr = head stk
          in go rest (drop 2 stk) mloadSrc (Just addr) cmpOps
    ...
    recordCmp stk rest mloadSrc mstoreA _ =
      let (a:b:rest') = stk
      in go rest (Computed : rest') mloadSrc mstoreA (Just (a, b))
