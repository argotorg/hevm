module EVM.Solvers.Internal (Solver(..), SolverGroup, checkSat, collectSolutions, withSolvers, MultiSol(..))
where
...
import Data.Text qualified as TStrict (pack)
import Data.Text.Internal.Builder (fromLazyText, toLazyText, fromString)
import Data.Text.Lazy (Text)
import Data.Text.Lazy qualified as T
import Data.Text.Lazy.IO qualified as T
import Data.Text.Lazy.Builder.Int qualified (decimal)
...
-- | Service able to check satisfiability
newtype SolverGroup = SolverGroup
  ( SMT2 -> Maybe [Prop] -> IO SMTResult -- single satisfiability check, uses cache if [Prop] is given
  , SMT2 -> MultiSol -> IO (Maybe [W256]) -- collects all solutions (up to given bound) for a given denoted variable
  )
...
data MultiData = MultiData
  { smt2 :: SMT2
  , multiSol :: MultiSol
  , resultVar :: MVar (Maybe [W256])
  }

data SingleData = SingleData
  { smt2 :: SMT2
  , props :: Maybe [Prop]
  , resultVar :: MVar SMTResult
  }
...
withSolvers solver count threads timeout cont = do
    ...
    res <- cont (SolverGroup (solveSingleTask taskq, collectSolutionsTask taskq))
...
checkSat :: SolverGroup -> Maybe [Prop] -> SMT2 -> IO SMTResult
checkSat (SolverGroup (runSolver, _)) props smt2 = runSolver smt2 props

collectSolutions :: SolverGroup -> SMT2 -> MultiSol -> IO (Maybe [W256])
collectSolutions (SolverGroup (_, collectSolutionsHandler)) = collectSolutionsHandler
...
getMultiSol :: forall m. (MonadIO m, ReadConfig m) => SMT2 -> MultiSol -> (MVar (Maybe [W256])) -> SolverInstance -> Chan SolverInstance -> Int -> m ()
...
getOneSol :: (MonadIO m, ReadConfig m) => SMT2 -> Maybe [Prop] -> MVar SMTResult -> TChan CacheEntry -> SolverInstance -> Chan SolverInstance -> Int -> m ()
getOneSol smt2@(SMT2 cmds cexvars _) props r cacheq inst availableInstances fileCounter = do
  ...
  liftIO $ do
    ...
    case out of
      ...
      Left e -> putMVar r (Error $ "Error while writing SMT to solver: " <> T.unpack e)
